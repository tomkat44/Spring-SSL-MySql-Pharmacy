...
public class PrescriptionControllerTest {
    ...
    public void findPrescriptionById() {
        ...
        prescription = when().get("http://localhost:8080/prescription/getSingle/53")
                .then()
                .statusCode(200)
                .extract().as(Prescription.class);

        Assertions.assertEquals(53, prescription.getId());
        Assertions.assertEquals(2, prescription.getQuantityPrescriptions().size());
        Assertions.assertEquals(3, prescription.getPrescriptionExecution().getId());
